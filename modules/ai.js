...
function openSolutionDetail(sector){
  ...
  const rows = data.items.map(it=>{
    ...
    if(p){
      ...
      return `<div class="finder-chip-row"><span>✔ ${it.label} × ${it.qty}</span><b>${p.name} — ${Number(p.price).toLocaleString('ar-SA')} ر.س/وحدة</b></div>`;
    }
  ...
  document.getElementById('solDetailTotal').innerText = total>0 ? `من ${total.toLocaleString('ar-SA')} ر.س` : 'يحتاج معاينة لتحديد السعر';
}
...
function runAiSolutionBuilder(){
  ...
  document.getElementById('aiBuilderTotal').innerText = total>0 ? `من ${total.toLocaleString('ar-SA')} ر.س` : 'يحتاج معاينة لتحديد السعر';
...
function renderFinderResults(){
  ...
  const recorderHtml = recorder.product
    ? `<p>الجهاز المقترح من كتالوجنا: <b>${recorder.product.name}</b> - ${Number(recorder.product.price).toLocaleString('ar-SA')} ر.س</p>`
