...
function initBoqBuilder(){
  ...
  if(picker && picker.dataset.filled !== '1'){
    picker.innerHTML = '<option value="">➕ اختر منتج من الكتالوج لإضافته...</option>' + products.map((p,i)=>`<option value="${i}">${p.name} — ${p.price?Number(p.price).toLocaleString('ar-SA')+' ر.س':''}</option>`).join('');
...
function renderBoqTable(){
  ...
  const t = boqTotals();
  document.getElementById('boqSubtotal').innerText = `${t.subtotal.toLocaleString('ar-SA')} ر.س`;
  document.getElementById('boqDiscountVal').innerText = `- ${t.discountVal.toLocaleString('ar-SA')} ر.س`;
  document.getElementById('boqVatVal').innerText = `+ ${t.vatVal.toLocaleString('ar-SA')} ر.س`;
  document.getElementById('boqGrandTotal').innerText = `${t.grandTotal.toLocaleString('ar-SA')} ر.س`;
}
...
function boqDownloadPdf(){
  ...
  doc.setFontSize(10);
  doc.text(`Subtotal: ${t.subtotal.toLocaleString('en-US')} SAR`, 14, y);
  doc.text(`Discount: -${t.discountVal.toLocaleString('en-US')} SAR`, 14, y+6);
  doc.text(`VAT: +${t.vatVal.toLocaleString('en-US')} SAR`, 14, y+12);
  doc.setFontSize(12);
  doc.text(`Grand Total: ${t.grandTotal.toLocaleString('en-US')} SAR`, 14, y+20);
...
function boqSendWhatsapp(){
  ...
  rows.forEach(i=>lines.push(`- ${i.name} × ${i.qty} = ${(i.qty*i.price).toLocaleString('ar-SA')} ر.س`));
  lines.push('', `الإجمالي بعد الخصم والضريبة: ${t.grandTotal.toLocaleString('ar-SA')} ر.س`);
...
function renderQuoteTable(){
  ...
  document.getElementById('qbGrandTotal').innerText = `${total.toLocaleString('ar-SA')} ر.س`;
}
function qbSendWhatsapp(){
  ...
  qbItems.filter(i=>i.name).forEach(i=>lines.push(`- ${i.name} × ${i.qty} = ${(i.qty*i.price).toLocaleString('ar-SA')} ر.س`));
  lines.push('', `الإجمالي التقريبي: ${total.toLocaleString('ar-SA')} ر.س`);
  window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(lines.join('\n'))}`, '_blank');
}
...
function qbPrintDocument(){
  ...
  const rowsHtml = rows.map(i => `
    <tr>
      ...
      <td style="text-align:center;">${Number(i.price).toLocaleString('ar-SA')} ر.س</td>
      <td style="text-align:center;font-weight:700;">${(i.qty*i.price).toLocaleString('ar-SA')} ر.س</td>
    </tr>`).join('');
  ...
  document.getElementById('qbPrintDoc').innerHTML = `
    ...
    <table class="qb-doc-table">
      ...
      <tfoot><tr class="qb-doc-total-row"><td colspan="3">الإجمالي الكلي</td><td style="text-align:center;">${total.toLocaleString('ar-SA')} ر.س</td></tr></tfoot>
    </table>
