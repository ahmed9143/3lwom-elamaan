...
function calcProjectCost(){
  ...
  document.getElementById('calcResultValue').innerText = `${min.toLocaleString('ar-SA')} - ${max.toLocaleString('ar-SA')} ر.س`;
  box.style.display = 'block';
  ...
  lastCalcSummary = `تقدير مبدئي للمشروع (${parts.join(' + ')}): ${min.toLocaleString('ar-SA')} - ${max.toLocaleString('ar-SA')} ر.س تقريبًا (توريد وتركيب، تقدير غير نهائي).`;
}
