  ...
  container.innerHTML = items.map(p=>{
    ...
    return `<div class="cart-item">
      ...
      <div class="cart-item-details" style="flex:1;">
        ...
        <div class="cart-item-price">${Number(p.price).toLocaleString('ar-SA')} ر.س</div>
...
async function applyCoupon(){
  ...
  if(data.min_order && subtotal < data.min_order){ msgEl.innerText = `الحد الأدنى للطلب ${Number(data.min_order).toLocaleString('ar-SA')} ر.س`; msgEl.classList.add('err'); return; }
  appliedCoupon = data;
...
function updateCartUI(){
  ...
  document.getElementById('cartTotalHeader').innerText = total.toLocaleString('ar-SA')+' ر.س';
  document.getElementById('cartDrawerTotal').innerText = total.toLocaleString('ar-SA')+' ر.س';
  document.getElementById('cartSubtotalRow').style.display = discount>0 ? 'flex' : 'none';
  document.getElementById('cartSubtotalVal').innerText = subtotal.toLocaleString('ar-SA')+' ر.س';
  document.getElementById('cartDiscountRow').style.display = discount>0 ? 'flex' : 'none';
  document.getElementById('cartDiscountVal').innerText = '- '+discount.toLocaleString('ar-SA')+' ر.س';
    ...
    container.innerHTML = Object.keys(cart).map(id=>{
      ...
      return `<div class="cart-item">
        <div class="cart-item-details">
          <div class="cart-item-title">${p.name}</div>
          <div class="cart-item-price">${(p.price*cart[id]).toLocaleString('ar-SA')} ر.س</div>
...
async function completeWhatsappOrder(custInfo){
  ...
  itemsSnapshot.forEach(it=>{
    msg += `▪️ *${it.name}*\n   العدد: ${it.qty} | إجمالي: ${(it.price*it.qty).toLocaleString('ar-SA')} ر.س\n\n`;
  });
  msg += "-------------------------\n";
  if(result.discount>0) msg += `الخصم${appliedCoupon?' ('+appliedCoupon.code+')':''}: -${Number(result.discount).toLocaleString('ar-SA')} ر.س\n`;
  msg += `💰 *الإجمالي:* ${Number(result.total).toLocaleString('ar-SA')} ر.س\n🔖 *مرجع الطلب:* ${result.order_ref}\n`;
...
async function confirmBankOrder(){
  ...
  msg += "-------------------------\n";
  if(result.discount>0) msg += `الخصم: -${Number(result.discount).toLocaleString('ar-SA')} ر.س\n`;
  msg += `💰 *الإجمالي:* ${Number(result.total).toLocaleString('ar-SA')} ر.س\n🔖 *مرجع الطلب:* ${result.order_ref}\n`;
...
async function lookupOrder(){
  ...
  try{
    ...
    const itemsHtml = (order.items||[]).map(it=>`<div class="row"><span>${it.name} × ${it.qty}</span><span>${(it.price*it.qty).toLocaleString('ar-SA')} ر.س</span></div>`).join('');
    let downloadHtml = '';
    ...
    box.innerHTML = `
      <div class="track-result">
        ...
        ${itemsHtml}
        <div class="row"><span>الإجمالي</span><b>${Number(order.total).toLocaleString('ar-SA')} ر.س</b></div>
        <div class="row"><span>تاريخ الطلب</span><span>${new Date(order.created_at).toLocaleDateString('ar-SA')}</span></div>
      </div>
